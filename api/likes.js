  ...
  export default async function handler(req, res) {
    ...
    try {
      if (req.method === 'GET') {
        ...
        const { data, error } = await supabase.from('likes').select('*').eq('user_id', user_id).order('created_at', { ascending: false });
      ...
      if (req.method === 'POST') {
        ...
        if (existing) { await supabase.from('likes').delete().eq('user_id', user_id).eq('song_id', song_id); return res.status(200).json({ liked: false }); }
      ...
      if (req.method === 'DELETE') {
        const { user_id, song_id } = req.body;
        const { error } = await supabase.from('likes').delete().eq('user_id', user_id).eq('song_id', song_id);
        if (error) throw error;
