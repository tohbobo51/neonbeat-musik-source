...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const { user_id, id, public_only } = req.query;
      ...
      // Only return public playlists when viewing someone else's profile
      if (public_only === 'true') {
        // Try filtering by visibility, fallback to all if column doesn't exist
        try {
    ...
    if (req.method === 'POST') {
      const { action, user_id, name, cover_url, playlist_id, song_id, visibility } = req.body;
      if (action === 'add_song') {
