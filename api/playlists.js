...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (id) {
        // Detail playlist beserta lagunya
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'add_song') {
        const { data: existing } = await supabase
          .from('playlist_songs').select('id').eq('playlist_id', playlist_id).eq('song_id', song_id).single();
        if (existing) return res.status(200).json({ ok: true, already: true });

        const { data: maxPos } = await supabase
          .from('playlist_songs').select('position').eq('playlist_id', playlist_id)
          .order('position', { ascending: false }).limit(1).single();
        const position = (maxPos?.position || 0) + 1;

        const { data, error } = await supabase
          .from('playlist_songs').insert({ playlist_id, song_id, position }).select().single();
        if (error) throw error;
