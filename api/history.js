...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Hapus entri lama dulu supaya tidak duplikat
      await supabase.from('listen_history').delete().eq('user_id', user_id).eq('song_id', song_id);
      const { data, error } = await supabase
        .from('listen_history')
        .insert({ user_id, song_id, played_at: new Date().toISOString() })
        .select()
        .single();
      if (error) throw error;
      // Tambah play count
      ...
      if (song) {
        await supabase.from('songs').update({ play_count: (song.play_count || 0) + 1 }).eq('id', song_id);
      }
