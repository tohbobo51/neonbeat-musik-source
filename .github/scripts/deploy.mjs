...
if (!token || !projectId) {
  console.error('Missing env vars: VERCEL_TOKEN, VERCEL_PROJECT_ID');
  process.exit(1);
}
...
for (const f of files) {
  ...
  if (res.status === 409) cached++;
    ...
    console.error('Upload failed:', f.path, res.status, t.slice(0, 100));
    process.exit(1);
  }
}
...
if (!deployRes.ok) {
  console.error('Deploy failed:', JSON.stringify(deploy));
  process.exit(1);
}
